...
func EZTxBuilder(txFrom string, addressFrom string, index uint32, addressTo string, fingerprint string, valueOut int64) *wire.MsgTx {
	...
	hashStr := txFrom // put the input txid here 
	...
	outPoint := wire.NewOutPoint(
		outpointTxid, index) // replace 0 with the output you want to spend
	...
	sendToAddressString := addressTo // put the address you're sending to here
	...
	output := wire.NewTxOut(valueOut, sendToScript) // replace 0 with the output value
	...
	prevAddressString := addressFrom // put the address you're sending "from" here
